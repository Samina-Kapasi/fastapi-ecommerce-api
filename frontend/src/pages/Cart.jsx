...
export default function Cart(){
    ...
    if(items.length===0)
        return <Box sx={{textAlign:"center",mt:10}}>
            ...
            <Button component={Link} to="/products" variant="contained" sx={{mt:3}}>Continue Shopping</Button>
            </Box>;
    return <Box sx={{p:4,bgcolor:"#f5f5f5",minHeight:"100vh"}}>
        <Typography variant="h4" fontWeight="bold" mb={3}>Shopping Cart</Typography>
        <Grid container spacing={3}>
            <Grid item xs={12} md={8}>{items.map(i=><Card key={i.cart_id} sx={{mb:2}}><Grid container><Grid item xs={12} sm={4}><CardMedia component="img" height="220" image={i.image||"https://via.placeholder.com/300x220?text=No+Image"}/>
            </Grid>
            <Grid item xs={12} sm={8}><CardContent>
                ...
                <Grid item xs={12} md={4}><Paper sx={{p:3}}>
                    ...
                    <Divider sx={{my:2}}/><Typography fontWeight="bold">Grand Total: ₹ {total}</Typography><Button fullWidth variant="contained" sx={{mt:3}}>Proceed To Checkout</Button></Paper></Grid></Grid></Box>;
}
